...
__device__ void dUpdate(int st[], int dp[], bool worklist[], int arg, int var, int
        callsite, int rowSize, int wlSize) {
  int idv = var * rowSize;
  int ida = arg * rowSize;
  int sv = st[idv];
...
__device__ void handleSite(int st[], int ar1[], int ar2[], int cf[], int dp[],
    ...
    for(int i = idf + 1; i < idf + nef; i++) {
      ...
      dUpdate(st, dp, newWl, arg1, var1, callSite, rowSize, wlSize);
      dUpdate(st, dp, newWl, arg2, var2, callSite, rowSize, wlSize);
    }
}
...
__global__ void dRunIter(int st[], int ar1[], int ar2[], int cf[], int dp[], int
